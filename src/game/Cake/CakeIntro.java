...
""",
            """
겁쟁이 병정 (다이아몬드 A):
자, 잠깐! 요리 중간에 방심하면 안 돼!
갑자기 '거대 딸기'가 떨어질 수도 있어!
[클릭]으로 거대 딸기를 마구 찔러줘야해
정신 바짝 차려!
""",
            // 3단계
            """
...
"""
    ...
    private void playAlienVoice() {
        try {
            ...
            alienVoice = new Music("card_voice2.mp3", false);
            alienVoice.start();
        } catch (Exception ex) {
            System.err.println("카드 보이스 재생 실패: " + ex.getMessage());
        }
    }
    ...
    private void drawWrappedStringWithHighlight(Graphics g, String text,
        ...
        for (String lineTextRaw : lines) {
            ...
            // 🔁 화자 줄 체크
            boolean isSpeakerLine = lineText.startsWith("겁쟁이 병정 (다이아몬드 A):");
